...
if(isset($_POST['sub'])){    
	$altname = mysqli_real_escape_string($conn,$_POST['altname']);
	...
	if($your_img != ''){
        ...
        else 
        {
            ...
            $b = "INSERT INTO `tc_banner`( `altname`, `image`, `date_time`, `date`) VALUES ('$altname','$your_imgtt','$date_time','$date')";
            mysqli_query($conn,$b);
            echo "<script>alert('Banner added Successfully!');</script>";
	        echo "<script>window.location.href='bannerslider.php';</script>";
        }
	}else{
		$b = "INSERT INTO `tc_banner`( `altname`, `date_time`, `date`) VALUES ('$altname','$date_time','$date')";
        mysqli_query($conn,$b);
        echo "<script>alert('Banner added Successfully!');</script>";
	    echo "<script>window.location.href='bannerslider.php';</script>";
	}
...
if(isset($_POST['upd'])){
	$id = $_POST['upd'];
	$tt = "SELECT * FROM `tc_banner` WHERE `id` = '$id'";
	$ee = mysqli_query($conn,$tt);
	$qq = mysqli_fetch_array($ee);
}else{
	$id = '';
...
if(isset($_POST['update']))
{
    ...
    if($your_img != '')
    {
        ...
        else 
        {
             ...
             $tt5 = "SELECT * FROM `tc_banner` WHERE `id` = '$id'";
             $ee5 = mysqli_query($conn,$tt5);
             $qq5 = mysqli_fetch_array($ee5);
             unlink('../../images/sliders/'.$qq5['image']);
             move_uploaded_file($_FILES['your_img']['tmp_name'],'../../images/sliders/'.$your_imgtt);
             $b = "UPDATE `tc_banner` SET `altname`='$altname',`image`='$your_imgtt',`date_time`='$date_time',`date`='$date' WHERE `id` = '$id'";
             mysqli_query($conn,$b);
             echo "<script>alert('Update successfully!');</script>";
             echo "<script>window.location.href='bannerslider.php';</script>";
        }
    }else{
        $b = "UPDATE `tc_banner` SET `altname`='$altname',`date_time`='$date_time',`date`='$date' WHERE `id` = '$id'";
        mysqli_query($conn,$b);
        echo "<script>alert('Update successfully!');</script>";
        echo "<script>window.location.href='bannerslider.php';</script>";
    }
...
if(isset($_POST['del'])){
	...
	$tt = "DELETE FROM `tc_banner` WHERE `id` = '$id'";
