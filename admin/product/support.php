...
if(isset($_POST['sub'])){    
	...
	if($your_img != ''){
        ...
        {
            move_uploaded_file($_FILES['your_img']['tmp_name'],'../../images/support/'.$your_imgtt);
            $b = "INSERT INTO `tc_support`( `title`,`link`, `image`, `date_time`, `date`) VALUES ('$title','$link','$your_imgtt','$date_time','$date')";
            mysqli_query($conn,$b);
            echo "<script>alert('Support file inserted successfully!');</script>";
	        echo "<script>window.location.href='support.php';</script>";
        }
	}else{
		$b = "INSERT INTO `tc_support`( `title`,`link`, `date_time`, `date`) VALUES ('$title','$link','$date_time','$date')";
        mysqli_query($conn,$b);
        echo "<script>alert('Support file inserted successfully!');</script>";
	    echo "<script>window.location.href='support.php';</script>";
	}
...
if(isset($_POST['upd'])){
	$id = $_POST['upd'];
	$tt = "SELECT * FROM `tc_support` WHERE `id` = '$id'";
	$ee = mysqli_query($conn,$tt);
	$qq = mysqli_fetch_array($ee);
}else{
	$id = '';
...
if(isset($_POST['update']))
{
    ...
    if($your_img != '')
    {
        ...
        else 
        {
             ...
             $tt5 = "SELECT * FROM `tc_support` WHERE `id` = '$id'";
             $ee5 = mysqli_query($conn,$tt5);
             $qq5 = mysqli_fetch_array($ee5);
             unlink('../../images/support/'.$qq5['image']);
             move_uploaded_file($_FILES['your_img']['tmp_name'],'../../images/support/'.$your_imgtt);
             $b = "UPDATE `tc_support` SET `title`='$title',`link`='$link',`image`='$your_imgtt',`date_time`='$date_time',`date`='$date' WHERE `id` = '$id'";
             mysqli_query($conn,$b);
             echo "<script>alert('Update successfully!');</script>";
             echo "<script>window.location.href='support.php';</script>";
        }
    }else{
        $b = "UPDATE `tc_support` SET `title`='$title',`link`='$link',`date_time`='$date_time',`date`='$date' WHERE `id` = '$id'";
        mysqli_query($conn,$b);
        echo "<script>alert('Update successfully!');</script>";
        echo "<script>window.location.href='support.php';</script>";
    }
...
if(isset($_POST['del'])){
	...
	$tt = "DELETE FROM `tc_support` WHERE `id` = '$id'";
	...
	<section class="content">
		<div class="row">
			...
			<div class="col-md-10">
				<div class="box">
					<div class="box-header">
						<h3 class="box-title"><b>Support</b></h3>
					</div>
						...
						<form class="" method="post" enctype="multipart/form-data">
                            ...
                            <div class="col-md-4">
                                ...
                                <label for="exampleInputEmail1">Link</label>
                                    <input type="text" class="form-control" name="link" placeholder="Link" value="<?php if($id != ''){ echo $qq['link']; } ?>">
                                </div>  
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                            ...
                            {?>
                                <div class="col-md-3" style="">	
                                    <?php 
                                        if($qq['image'] !='') 
                                        { ?>
                                            <img src="../../images/support/<?php echo $qq['image']; ?>" style="width:120px">
                                            <?php 
                                        } else
                                        {
							...
							<div class="col-md-12" style=""><br>
                                ...
                                <p>- Image dimension must be 200 * 200 pixel.</p>
                                <hr>
                            </div>
            ...
            <div class="col-md-12">	
				<div class="box">
					<div class="box-header">
						<h3 class="box-title"><b>Manage Support</b></h3>
					</div>
					<div class="box-body box box-info">
							...
							<tbody>
								...
								<tr>
									...
									<td>
                                        ...
                                        if($nn1['image'] !='') 
                                        { ?>
                                            <img src="../../images/support/<?php echo $nn1['image']; ?>" style="width:120px">
                                            <?php 
                                        } else
                                        {
                                            echo 'Image not found.';
